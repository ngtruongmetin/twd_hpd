...
async function sendPasswordResetEmail({ toEmail, username, password, fullName }) {
  const transporter = createGmailTransporter();

  const subject = "Mật khẩu mới - Yêu cầu đặt lại";
  const safeFullName = escapeHtml(fullName || toEmail);
  const safeUsername = escapeHtml(username);
  const safePassword = escapeHtml(password);
  const text = [
    ...
    "",
    "Vui lòng đăng nhập bằng tài khoản mới này để tiếp tục sử dụng hệ thống.",
  ].join("\n");
    ...
    <body style="margin:0;padding:0;background:#fafafa;font-family:'Be Vietnam Pro','Segoe UI',Tahoma,Arial,sans-serif;color:#111">
      <div style="max-width:680px;margin:0 auto;padding:28px 18px">
        <div style="border:1px solid #e5e5e5;border-top:5px solid #ef4444;background:#fff;border-radius:16px;overflow:hidden">
          <div style="padding:24px 28px;border-bottom:1px solid #e5e5e5">
            <img src="cid:chuhieu" alt="Hoa Phượng Đỏ" style="display:block;width:220px;max-width:100%;height:auto;margin:0 auto 20px" />
            <p style="margin:0 0 8px;font-size:12px;line-height:1.5;font-weight:700;color:#ef4444;text-transform:uppercase;letter-spacing:.08em">Đặt lại mật khẩu</p>
            <h1 style="margin:0;font-size:28px;line-height:1.25;font-weight:800;color:#111">Mật khẩu của bạn đã được đặt lại</h1>
            <p style="margin:14px 0 0;font-size:16px;line-height:1.75;color:#525252">Xin chào <strong style="color:#111">${safeFullName}</strong>, mật khẩu của bạn đã được đặt lại thành công.</p>
          </div>
          <div style="padding:24px 28px">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
              <tr>
              ...
              <tr>
                <td style="padding:12px 0;color:#a3a3a3;font-size:12px;font-weight:700;text-transform:uppercase">Mật khẩu mới</td>
                <td style="padding:12px 0;text-align:right;font-weight:700">${safePassword}</td>
              </tr>
            </table>
            <div style="margin-top:20px;padding:16px;border:1px solid #e5e5e5;border-radius:12px;background:#fafafa">
              <p style="margin:0;color:#111;font-size:15px;line-height:1.7">Vui lòng đăng nhập bằng tài khoản mới này để tiếp tục sử dụng hệ thống. Nếu không phải bạn gửi yêu cầu, vui lòng liên hệ BTC.</p>
            </div>
          </div>
        </div>
      </div>
