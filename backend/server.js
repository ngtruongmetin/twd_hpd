...
function ensureVirtualSubmissionOwnerNullable() {
  db.all("PRAGMA table_info(submissions)", [], (err, rows) => {
    if (
      ...
      if (err) {
        console.error(
          "Failed to inspect submissions owner constraint:",
          err.message
        );
      }
      return;
    ...
    db.serialize(() => {
      ...
      db.run(
        "DROP TRIGGER IF EXISTS trg_submissions_max_3",
        (dropErr) => {
          if (dropErr) {
            console.error(
              "Failed to drop submissions trigger during migration:",
              dropErr.message
            );
          }
        }
      ...
      db.run(
        "ALTER TABLE submissions RENAME TO submissions_legacy_owner_migration",
        (renameErr) => {
          if (renameErr) {
            console.error(
              "Failed to rename submissions during migration:",
              renameErr.message
            );
            db.run("ROLLBACK");
            return;
          ...
          `, (createErr) => {
            if (createErr) {
              console.error(
                "Failed to recreate submissions during migration:",
                createErr.message
              );
              db.run("ROLLBACK");
              return;
            ...
            db.run(`
              ...
              SELECT
                ...
                is_failed,
                failed_reason
              FROM submissions_legacy_owner_migration
            `, (copyErr) => {
              if (copyErr) {
                console.error(
                  "Failed to copy submissions during migration:",
                  copyErr.message
                );
                db.run("ROLLBACK");
                return;
              ...
              db.run(
                "DROP TABLE submissions_legacy_owner_migration",
                (dropTableErr) => {
                  if (dropTableErr) {
                    console.error(
                      "Failed to remove legacy submissions table:",
                      dropTableErr.message
                    );
                    db.run("ROLLBACK");
                    return;
                  }
                  ...
                  db.run(
                    "CREATE INDEX IF NOT EXISTS idx_submissions_competition_table_id ON submissions(competition_table_id)"
                  );

                  db.run(
                    "CREATE INDEX IF NOT EXISTS idx_submissions_submitted_by_user_id ON submissions(submitted_by_user_id)"
                  );

                  db.run(
                    "CREATE INDEX IF NOT EXISTS idx_submissions_season_table_status ON submissions(season_id, competition_table_id, status)"
                  );

                  db.run("COMMIT", (commitErr) => {
                    if (commitErr) {
                      console.error(
                        "Failed to commit submissions owner migration:",
                        commitErr.message
                      );
                    } else {
                      console.log(
                        "Made submissions.submitted_by_user_id nullable"
                      );
                    }
                  });
                }
